...
function DeleteTaskModal({ isOpen, onClose, onDeleteTask, task }: { isOpen: boolean; onClose: () => void; onDeleteTask: () => void; task?: Task | null }) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <div className="z-10 w-full max-w-[560px] rounded-[28px] border border-slate-200 bg-white p-6 shadow-xl sm:p-8">
                <div className="space-y-3">
                    <p className="text-lg font-semibold text-slate-900">Delete this task?</p>
                    <p className="text-sm leading-6 text-slate-600">This will remove the task from your board permanently.</p>
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">{task?.title || "Selected task"}</div>
                </div>
